...
class Weather(Base_Cog):
  ...
  async def set_weather_place(self, inter: disnake.CommandInteraction, *, place: str=commands.Param(description="Place where to request weather for")):
    if not _place_is_valid(place):
      return message_utils.generate_error_message(inter, Strings.weather_set_place_invalid_place)

    weather_settings_repo.set_weather_settings(inter.author.id, place)
    await message_utils.generate_success_message(inter, Strings.weather_set_place_set(place=place))
  ...
  async def unset_weather_place(self, inter: disnake.CommandInteraction):
    if not weather_settings_repo.remove_weather_settings(inter.author.id):
      return await message_utils.generate_error_message(inter, Strings.weather_unset_place_not_place_to_remove)
    await message_utils.generate_success_message(inter, Strings.weather_unset_place_removed)
  ...
  async def weather_request(self, inter: disnake.CommandInteraction, place: Optional[str]=commands.Param(default=None, description="Place where to request weather for")):
    ...
    if place is None:
      return await message_utils.generate_error_message(inter, Strings.weather_request_weather_place_not_set)
    ...
    if embeds is None:
      return await message_utils.generate_error_message(inter, Strings.weather_request_weather_error)
